    ...
    public static void main(String[] args) {
        try {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                int[][] Sparse1 = new int[nums.length][3];

                for (int r = 0; r < mat1.length; r++) {
                    for (int c = 0; c < mat1[r].length; c++) {
                        if (mat1[r][c] != 0) {
                            Sparse1[Sparse1.length - x][0] = r + 1;
                            Sparse1[Sparse1.length - x][1] = c + 1;
                            Sparse1[Sparse1.length - x][2] = mat1[r][c];
                            x--;
                        }
                    }
                }
                ...
                if ((Sparse1.length) == (mat1.length + mat1[1].length)) {
                    System.out.println("The Original Matrix and the Sparse Matrix are Equally Efficient");
                } else if ((Sparse1.length) > (mat1.length + mat1[1].length)) {
